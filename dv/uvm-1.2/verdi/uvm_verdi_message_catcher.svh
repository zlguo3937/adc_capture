...
`ifdef VERDI_MSG_PARSE_DPI
     // void'(parse_phase_msg(phase_full_name, domain, schedule, phase)); // original flow 

        chandle domain_ptr, schedule_ptr, phase_ptr;

        void'(parse_phase_msg_by_chandle(phase_full_name, domain_ptr, schedule_ptr, phase_ptr));
        
        domain = verdi_dpi_get_c_string_by_chandle(domain_ptr);
        schedule = verdi_dpi_get_c_string_by_chandle(schedule_ptr);
        phase = verdi_dpi_get_c_string_by_chandle(phase_ptr);
        
        verdi_dpi_free_c_string_by_chandle(domain_ptr);
        domain_ptr = null; 
        verdi_dpi_free_c_string_by_chandle(schedule_ptr);
        schedule_ptr = null;
        verdi_dpi_free_c_string_by_chandle(phase_ptr);
        phase_ptr = null; 
...
`endif
     ...
     function void set_message_attribute_int(longint unsigned handle,string attrName,int value);
       string attr_name = "";
       longint unsigned st_handle=0, st_value=0;
...
`endif
//
       pli_inst.add_attribute_severity_type(handle,severity_type,attr_name);
     endfunction
...
`endif
//
       pli_inst.add_attribute_verbosity_type(handle,verbosity_type,attr_name);
     endfunction
     ...
     // Verdi P10049895-123644
     // For XVM, need to remove xvm_uvm_parent and xvm_ovm_parent
`ifdef XVM
     function string remove_xvm_parent_str (string ori_string);
         int position=0, position1=0;
         static string temp_string, revised_string;  

         temp_string = ori_string;
         position = find_substr(temp_string,"xvm_uvm_parent");
         // Don't record xvm_uvm_parent
         if (position>=0&&temp_string.len()==14)
             return "";
         while (position>=0 && temp_string.len()>14) begin
             revised_string = temp_string.substr(position+15,temp_string.len()-1);
             temp_string = revised_string;
             position1 = find_substr(temp_string,"xvm_ovm_parent");
             if (position1>=0 && temp_string.len()>14) begin
                 revised_string = temp_string.substr(position+15,temp_string.len()-1);
                 temp_string = revised_string;
             end
             position = find_substr(temp_string,"xvm_uvm_parent");
...
`endif
     ...
     virtual function action_e catch();
         string file_name = "", label = "", message = "", stream = "", id = "", revised_stream = "";
         string implicit_stream = "";
         int severity = 0;
         ...
         longint unsigned streamId = 0, handle = 0, endedId = 0; // Verdi P10049895-123644 
         ...
         integer r_stream_h=0;
         string reg_full_name="",value_s="",address_s="",mirrored_s="",desired_s="",event_type="",path_s="",map_s="",door_s=""; // 9001494539
         string comp_label,comp_message,comp_severity,comp_stream,comp_name;
         string comp_full_name,comp_type_name,comp_full_type_name,comp_caller_name,comp_provider_name,revised_comp_full_name,hierarchy_type,real_hierarchy_name; // Verdi P10049895-123644
         int objtn_maximum_option_check = 0;
         int rsrc_option_check = 0;
         int is_rsrc_inc_internal = 0;
         ...
         begin
           ...
           real_hierarchy_name = comp_full_name; // Verdi P10049895-123644 
...
`ifndef VERDI_REPLACE_DPI_WITH_PLI
           hierarchy_type = "xvm";
           comp_full_name = remove_xvm_parent_str(comp_full_name);
           if (comp_full_name=="")
               return THROW;
           endedId = pli_inst.get_ended_stream_id(comp_stream);
           if(!endedId) begin
                ...
                pli_inst.add_dense_attribute_string(streamId,hierarchy_type,"hierarchy_type");
...
`endif
           ...
           set_message_attribute_str(handle,"hierarchy_type",hierarchy_type);
           set_message_attribute_str(handle,"real_hierarchy_name",real_hierarchy_name);
           pli_inst.end_tr(handle);
...
`ifdef XVM
           hierarchy_type = "xvm";
           comp_caller_name = remove_xvm_parent_str(comp_caller_name);
           if (comp_caller_name=="")
               return THROW;
           comp_provider_name = remove_xvm_parent_str(comp_provider_name);
           if (comp_provider_name=="")
               return THROW;
`ifndef VERDI_REPLACE_DPI_WITH_PLI
           endedId = pli_inst.get_ended_stream_id(comp_stream);
...
`endif
         ...
         begin
           ...
           begin
             ...
             position2 = find_substr(message,", ");
             if(position2 >= 0)
               description = message.substr(position2+2,position1-1);
             else if(position2 == -1) begin
               position2 = find_substr(message,"objection(s) (");
               if(position2 >= 0)
                 description = message.substr(position2+14,position1-1);
             end 
             if(position2 == -1) position2 = position1;
...
`endif
           ...
           string stream_type = "register",des_str="";
           ...
           if (event_type=="Peeked" || event_type=="Poked") begin
               ...
               if (position>0) begin
                   ...
                   // 9001494539 Get map 
                   map_s = path_s; 
                   door_s = "UVM frontdoor";
               end else begin
                   // 9001494539 Get frontdoor/backdoor
                   position2 = find_substr(message,"via");
                   if(position2>0) begin 
                     door_s = message.substr(position2+4,position1-1);

                       if( (position=find_substr(message,"DPI")) > position2)
                           path_s = message.substr(position+4, position1-1);
                       else if ( (position=find_substr(message, "user")) > position2)
                           path_s = message.substr(position+5, position1-1);
                       else
                           path_s = message.substr(position2+4, position1-1);
                   end 
               end
           end
           begin
              ...
              if (!_mapr_inst.end_recording(event_type, path_s, value_s, reg_full_name, map_s, door_s)) begin // 9001494539
                 if (reg_full_name!="")
...
// P80000979-140209
              void'($sscanf(address_s, "'h%h", hex_address));
...
//
           ...
           string env_meg_length_limit_str = "";
           int env_meg_length_limit = 30;
           int meg_length_limit_max = 1000000;
           if( verdi_clp.get_arg_value("+UVM_MESSAGE_LENGTH_LIMIT=", env_meg_length_limit_str))
               env_meg_length_limit = env_meg_length_limit_str.atoi();
           if (message.len() > env_meg_length_limit) begin
               if (message.len() > meg_length_limit_max) begin
                   if (env_meg_length_limit > meg_length_limit_max || env_meg_length_limit ==0 ) 
                      position = meg_length_limit_max-1;
                   else
                      position = env_meg_length_limit-1;
                      label = {message.substr(0,position)," ..."};
               end else begin
                   if (env_meg_length_limit > meg_length_limit_max) begin
                      position = meg_length_limit_max-1;
                      label = {message.substr(0,position)," ..."};
                   end else if (env_meg_length_limit ==0) begin
                      position = message.len()-1;
                      label = message;
                   end else begin
                      position = env_meg_length_limit-1;
                      label = {message.substr(0,position)," ..."};
