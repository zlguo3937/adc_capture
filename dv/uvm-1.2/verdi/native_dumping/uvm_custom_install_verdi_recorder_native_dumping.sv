...
`include "./native_dumping/uvm_verdi_message_catcher_native_dumping.svh"
`include "./native_dumping/uvm_verdi_factory_native_dumping.svh"
...
`ifdef VERDI_DHIER_INTERFACE_BY_CLASS_DEFN
                /* New flow: 
                   Retrieve uvm_component's classdefn from its derived class uvm_dhier_component
                   Then, iterate classdefn to get all uvm components. */
                if(top_comp.get_name() == "DHIER_COMP") 
                    verdi_dhier_interface_by_classdefn("top_comp");
`else
                /* Old flow: (Default)
                   Get uvm components by traverse the whole hierarchy from test_top. */
                if(top_comp.get_name() != "DHIER_COMP") 
                    verdi_dhier_interface_by_comp_hier("top_comp");            
`endif
             end
         end    
`endif
...
`ifdef VCS
     if ((clp.get_arg_matches("+UVM_LOG_RECORD", tr_args)&& (!clp.get_arg_matches("+UVM_VPD_RECORD", tr_args)))
...
`ifdef VCS
       // 9001375412 Allow user to set dumping level
       if (verdi_clp.is_verdi_trace_ralwave()) begin
           static string val_str="";

           void'(verdi_clp.get_arg_value("+UVM_OBJ_DUMP_LEVEL=", val_str)); 
           if (is_vcs_uvm_home_exist)
               pli_inst.dump_class_object_by_file("${VCS_UVM_HOME}/verdi/register.config",val_str);
           else if (is_vcs_home_exist)
               pli_inst.dump_class_object_by_file("${VCS_HOME}/etc/uvm-1.2/verdi/register.config",val_str); 
       end 
       //
       // 9001375412 Allow user to set dumping level
       if (verdi_clp.is_verdi_trace_compwave()) begin
           ...
           void'(verdi_clp.get_arg_value("+UVM_OBJ_DUMP_LEVEL=", val_str));
           if (is_vcs_uvm_home_exist)
               pli_inst.dump_comp_object_by_file("${VCS_UVM_HOME}/verdi/component.config",val_str);
           else if (is_vcs_home_exist)
               pli_inst.dump_comp_object_by_file("${VCS_HOME}/etc/uvm-1.2/verdi/component.config",val_str);
       end
       //
`endif
...
`ifdef VCS
     if ((clp.get_arg_matches("+UVM_TR_RECORD", tr_args)&& (!clp.get_arg_matches("+UVM_VPD_RECORD", tr_args)))
...
`endif
   ...
   function void get_new_test();
     string test_name,comp_stream;
     ...
     pli_inst = uvm_verdi_pli::get_inst();
     $sformat(comp_stream,"UVM.HIER_TRACE.DTL");
     ...
     pli_inst.set_label(handle,"New Test");
     pli_inst.add_attribute_string(handle,test_name,"name","");
