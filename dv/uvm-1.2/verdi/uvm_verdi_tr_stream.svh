...
class uvm_verdi_tr_stream extends uvm_tr_stream;
  ...
  // P10049895-130603
   function bit mark_user_top_scope(string tr_str);
    int pos = 0, state = 0;
    string top_scope;

    for(pos=1; pos<tr_str.len(); ++pos) begin
      if(tr_str[pos] == ".") begin
        top_scope = tr_str.substr(0, pos-1);
`ifndef VERDI_REPLACE_DPI_WTIH_PLI
        pli_inst.add_scope_attribute(top_scope, "1", "SNPS_user_top");
`else
        pli_inst.add_scope_attribute(top_scope, "1", "+name+SNPS_user_top");
`endif
        return 1;
      end
    end
   ...
   protected virtual function void do_open(uvm_tr_database db,
       ...
       string stream_name="";
       string des_str, comp_str;
       static bit already_marked_user_top_scope = 0;
...
`endif
      ...
      if (m_verdi_db.open_db()) begin
        if (!streamArrByName.exists(scope)) begin
            ...
            if(!already_marked_user_top_scope && stream_id!=0)
                already_marked_user_top_scope = mark_user_top_scope(stream_name);
...
`endif
        ...
        if (hooks_version_flag==0) begin
            $display("Info: Verdi UVM-1.2 Hooks File 2022/05/26 ");
