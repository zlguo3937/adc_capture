...
virtual class uvm_reg extends uvm_object;
   ...
   // VCS Introduced event to resolve potential race of reading mirrored value when explicit predictor is called.
`ifdef UVM_VCS_EXT_PREDICT_SYNC
    uvm_event predict_done_event = uvm_event_pool::get_global("predict_done");
`endif
   ...
   // Function: set_reg_array_parent
   // 
   // Specify the parent block and parent register array.
   // It avoids calling blk.add_reg like configure function.
   // This is needed for uvm_reg_array & uvm_reg_array_md which
   // create a register after regmodel is locked.
   //
   extern function void set_reg_array_parent (uvm_reg_block blk_parent,
                                              uvm_reg_file reg_array_parent);
   `endif
   ...
   `ifdef UVM_FUNC_GET_MASK
     // Function: GET_MASK
     // Returns the width, in bits, of this register.
     extern virtual function int             unsigned GET_MASK ();
   `endif

   `ifdef UVM_SPARSE_ARRAY
   extern virtual function uvm_reg         allocate   ();
   `endif
...
`ifdef UVM_SPARSE_ARRAY
// set_reg_array_parent

function void uvm_reg::set_reg_array_parent (uvm_reg_block blk_parent,
                                             uvm_reg_file reg_array_parent);
   m_parent = blk_parent;
   m_regfile_parent = reg_array_parent;
endfunction: set_reg_array_parent
`endif
...
function void uvm_reg::do_predict(uvm_reg_item      rw,
   ...
   if(rw.status != UVM_HAS_X) rw.status = UVM_IS_OK;
...
`ifdef UVM_VCS_EXT_PREDICT_SYNC
            ...
            predict_done_event.wait_ptrigger();
`endif            
         end
      end
...
function int unsigned uvm_reg::GET_MASK();
   return 32'hFFFF_FFFF & 32'((1 << m_n_bits) - 1);
endfunction
`endif
...
function uvm_reg uvm_reg::allocate();
   `uvm_fatal("RegModel","uvm_reg::allocate() not implemented in derivative");
   allocate = null;
