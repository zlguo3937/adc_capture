...
virtual class uvm_reg_block extends uvm_object;
   ...
   // Function: create_banked map
   //
   // Create an address map of type snps_uvm_reg_map in this block
   //
   // Create an address map with the specified ~name~, then
   // configures it with the following properties.
   //
   ...
   //
   //| APB = create_map("APB", 0, 1, UVM_LITTLE_ENDIAN, 1);
   extern virtual function snps_uvm_reg_map create_snps_bank_map(string name,
                                                  uvm_reg_addr_t base_addr,
                                                  int unsigned n_bytes,
                                                  uvm_endianness_e endian,
                                                  bit byte_addressing = 1);
   ...
   `ifdef UVM_SPARSE_ARRAY
   /*local*/ extern function void add_reg_array   (uvm_reg_file  rg);
   `endif
   ...
   extern virtual function void get_registers (ref uvm_reg regs[$],
                                               input uvm_hier_e hier=UVM_HIER);
   //Function: get_registers_filtered_by_address
   //
   //get the registers filtered on the basis of address range specified by user   
`ifdef UVM_REG_ENABLE_ADDRESS_EXCLUSION   
   extern virtual function void get_registers_filtered_by_address (ref uvm_reg regs[$],
                                                      input uvm_hier_e hier = UVM_HIER); 
`endif   
   // Function: get_fields
   //
...
function void uvm_reg_block::add_reg_array(uvm_reg_file rg);
   if (this.is_locked()) begin
      `uvm_error("RegModel", "Cannot add register array to locked block model");
      return;
   end

   if (this.regarrays.exists(rg)) begin
      `uvm_error("RegModel", {"Register '",rg.get_name(),
         "' has already been registered with block '",get_name(),"'"})
       return;
   end
   ...
   regarrays[rg] = id++;
endfunction: add_reg_array
`endif //UVM_SPARSE_ARRAY
...
function void uvm_reg_block::lock_model();
   ...
   `ifdef UVM_SPARSE_ARRAY
   foreach (regarrays[rga_]) begin
      uvm_reg_file rga = rga_;
      rga.Xlock_modelX();
   end
   `endif
...
//This config class basically sets user defined address range for inclusion and exclusion of registers

`ifdef UVM_REG_ENABLE_ADDRESS_EXCLUSION
class uvm_reg_address_config extends uvm_object;

`uvm_object_utils(uvm_reg_address_config)

    function new(string name="uvm_reg_address_config");
        super.new(name);
    endfunction
...
uvm_reg_addr_range_t exc_range[$], inc_range[$];

    function bit add_address_exclude_range(uvm_reg_addr_t begin_addr,uvm_reg_addr_t end_addr);
        if(begin_addr >= end_addr)
          begin
          `uvm_warning("Regmodel",{"Range begin and end can't be same"});
          return 0;
         end
        if(inc_range.size() > 0)
          begin
           `uvm_warning("Regmodel",{"Exclude range cannot be applied as it is already part of include range"});
           return 0;
          end
        exc_range.push_back({begin_addr, end_addr});
        return 1;
    endfunction
    function bit add_address_include_range(uvm_reg_addr_t begin_addr,uvm_reg_addr_t end_addr);
        if(begin_addr >= end_addr)
          begin
           `uvm_warning("Regmodel",{"Range begin and end can't be same"});
           return 0;
          ...
          if(exc_range.size() > 0)
           begin
            `uvm_warning("Regmodel",{"Include range cannot be applied as it is already part of exclude range"});
             return 0;
           end
           inc_range.push_back({begin_addr, end_addr});
           return 1;
    endfunction

    function void clear_address_range();
        begin
        exc_range.delete();
        inc_range.delete();
       end
...
`ifdef UVM_REG_ENABLE_ADDRESS_EXCLUSION 
//This basically include or exclude registers on the basis of address range defined by user

function void uvm_reg_block::get_registers_filtered_by_address(ref uvm_reg regs[$],input uvm_hier_e hier = UVM_HIER);

uvm_reg_addr_t block_addr;
uvm_reg_addr_t reg_addr;
uvm_reg temp_regs[$];
uvm_reg_address_config cfg_obj = null;
uvm_reg_address_config::uvm_reg_addr_range_t exc[$], inc[$];
int delete_reg_ind[$];
get_registers(regs, hier);
if (uvm_config_db#(uvm_reg_address_config)::get(null, {"REG::", this.get_full_name()}, "UVM_REG_EXCLUSION_ADDRESS_RANGE", cfg_obj)) begin
    exc = cfg_obj.exc_range;
    inc = cfg_obj.inc_range;
    foreach(regs[i]) begin
        reg_addr = regs[i].get_offset();
        foreach(inc[j]) begin
        if((reg_addr < inc[j].min) || (reg_addr > inc[j].max)) begin
            delete_reg_ind.push_front(i);
            break;
           end
        end
        foreach(exc[j]) begin
        if((reg_addr >= exc[j].min) && (reg_addr <= exc[j].max)) begin
            delete_reg_ind.push_front(i);
            break;
           end
        end
    end
    foreach(delete_reg_ind[i])
        regs.delete(delete_reg_ind[i]);
end
endfunction
`endif
...
// create_snps_bank_map

function snps_uvm_reg_map uvm_reg_block::create_snps_bank_map(string name,
                                               uvm_reg_addr_t base_addr,
                                               int unsigned n_bytes,
                                               uvm_endianness_e endian,
                                               bit byte_addressing=1);
   ...
   map = snps_uvm_reg_map::type_id::create(name,,this.get_full_name());
   map.configure(this,base_addr,n_bytes,endian,byte_addressing);
