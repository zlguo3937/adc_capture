...
virtual class uvm_recorder extends uvm_object;
  ...
  bit max_array_limit_check = 0;
  longint max_array_num_limit = -1;
  // end
  ...
  // Modified by Verdi 9001505562
  function longint verdi_get_array_limit();
     if(max_array_limit_check==0) begin
        string val_str = "";
        max_array_limit_check = 1;
        if ($value$plusargs("UVM_ARRAY_NUM_LIMIT=%s", val_str))
            max_array_num_limit = val_str.atoi();
     end
     return max_array_num_limit;
  endfunction
  //
