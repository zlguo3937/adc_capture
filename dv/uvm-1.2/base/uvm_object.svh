...
`ifdef UVM_VCS_FGP
   typedef class uvm_fgp_thread_object; 
   class thread_specific_object;
   ...
   //uvm_fgp_thread_object#(thread_specific_object) fgp_thread_obj = null;
`endif
...
`ifdef UVM_CREATE_OPT 
    virtual function void call_new(); endfunction 
    virtual function uvm_object create (string name=""); 
       uvm_object res = $__vcs_new($__vcs_class_handle_obj(this)); 
       res.call_new(); 
       if (name!="") res.set_name(name); 
       return res; 
    endfunction
`else
...
`endif
 ...
 (* uvm_fgp_lock = "uvm_object::sprint" *)
...
`ifdef UVM_VCS_FGP
  m_inst_id = $fgpAtomicAddAndFetchInt(m_inst_count, 1);
`else
...
`ifdef UVM_VCS_FGP
  return $fgpAtomicAddAndFetchInt(m_inst_count, 0);
`else
  return m_inst_count;  
`endif
endfunction
