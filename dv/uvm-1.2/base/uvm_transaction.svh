...
function void uvm_transaction::do_record (uvm_recorder recorder);
  ...
  if (m_transaction_id != -1)
    recorder.record_field("transaction_id", m_transaction_id, $bits(m_transaction_id), UVM_DEC);
  // End 
...
`ifndef UVM_NO_VERDI_RECORD
        tr_recorder = stream_handle.open_recorder(get_name()!=""?get_name():get_type_name(),
                                                  this.begin_time,
