...
virtual class uvm_tr_stream extends uvm_object;
   ...
   function uvm_recorder open_recorder(string name,
                                      time   open_time = 0,
                                      string type_name="");
  // Modified by Verdi P10049895-133093, P10049895-142165
      time m_time;
`ifdef UVM_VERDI_FORCE_BEGIN_TR_TIME
      m_time = 0;
`else    
      // m_time = (open_time == 0 ) ? $time : open_time; 

      static bit b_use_realtime = ($test$plusargs("UVM_VERDI_BEGIN_TR_REALTIME")) ? 1 : 0;
      if(open_time != 0)
         m_time = open_time;
      else 
         m_time = b_use_realtime ? $realtime : $time; 
`endif
  // End
