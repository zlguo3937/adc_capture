  ...
  static function void add(T obj, uvm_callback cb, uvm_apprepend ordering=UVM_APPEND);
    ...
    if(obj == null) begin
      ...
      if(q.size() == 0) begin
        ...
        uvm_report_object o;
        `ifndef UVM_NO_GLOBAL_CATCHER_DUPL_CHK 
        int qn;
        `endif 
	  ...
	  void'(uvm_callbacks#(uvm_report_object, uvm_callback)::get());
        ...
        // Following change is done to avoid global catcher getting duplicated 
        // if instance-specific one is added later
        qn = q.size();
        for(int i=0; i<m_t_inst.m_tw_cb_q.size(); ++i) begin
           q.push_back(m_t_inst.m_tw_cb_q.get(i));
           // Don't add if already added by the implicit super::uvm_report_object
           for (int j = 0; j < qn; j++) begin
              if (q.get(j) == m_t_inst.m_tw_cb_q.get(i)) begin
                 void'(q.pop_back());
                 break;
              end
           end
