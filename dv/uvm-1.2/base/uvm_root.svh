...
function void uvm_root::m_do_verbosity_settings();
  ...
  // 9001487267
  if (verdi_verbosity_options!="") begin
    uvm_split_string(verdi_verbosity_options, ",", split_vals);
    if(split_vals.size() < 4 || split_vals.size() > 5) begin
      uvm_report_warning("INVLCMDARGS",
        $sformatf("Invalid number of arguments found on the command line for setting '+uvm_set_verbosity=%s'.  Setting ignored.",
        verdi_verbosity_options), UVM_NONE, "", "");
    end
    // Invalid verbosity
    if(!clp.m_convert_verb(split_vals[2], tmp_verb)) begin
      uvm_report_warning("INVLCMDVERB",
...
`else         
        "'b" : void'($sscanf(extval, "%b", v));
        "0b" : void'($sscanf(extval, "%b", v));
        "'o" : void'($sscanf(extval, "%o", v));
        "'d" : void'($sscanf(extval, "%d", v));
...
function void uvm_root::m_do_max_quit_settings();
 ...
 `else 
     void'($sscanf(split_max_quit[0],"%d", max_quit_int));
 `endif
    case((split_max_quit.size()>1) ? split_max_quit[1] : "default")
