...
`ifdef VCS extern function void uvm_set_verbosity(string setting_comp,string setting_id,uvm_verbosity setting_verbosity,string setting_phase,time setting_offset); `endif
`endif 
  // End
...
function uvm_component::new (string name, uvm_component parent);
  ...
  begin
    ...
    if (`UVM_VCS_CHECK(bld.get_state() == UVM_PHASE_DONE)) begin
...
`endif
  ...
  if(uvm_report_enabled(UVM_MEDIUM+1, UVM_INFO, "NEWCOMP"))
    ...
    if (`UVM_VCS_CHECK(parent.has_child(name) && this != parent.get_child(name))) begin
    if (parent == top) begin
...
`ifndef UVM_NO_VERDI_RECORD
`ifdef VCS
  if (is_verdi_set_verbosity_called)
      uvm_set_verbosity(setting_comp,setting_id,setting_verbosity,setting_phase,setting_offset);
`endif
`endif
  //
endfunction
...
function bit uvm_component::m_add_child(uvm_component child);
  ...
  if (`UVM_VCS_CHECK(m_children.exists(child.get_name()) &&
    ...
    if(`UVM_VCS_CHECK(m_children_by_handle.exists(child))) begin
      `uvm_warning("BDCHLD",
   ...
   if (uvm_verbosity'(recording_detail) != UVM_NONE) begin
      if ((stream_name == "") || (stream_name == "main")) begin
        ...
        if (is_verdi_recorder_enabled()) begin
            t_str = check_component_type(this);
            stream = get_tr_stream(stream_name,t_str);
      ...
      if (stream != null ) begin
         ...
         `ifndef UVM_VCS_RECORD
            ...
            $psprintf("Object ID: %0s (@%0d)\n", db.get_object_id(tr), tr.get_inst_id()),
            $psprintf("Sequencer ID: %0s (@%0d)\n", db.get_object_id(this), get_inst_id())};
...
// Modified by Verdi 9001092057 9001330016
`ifndef UVM_NO_VERDI_RECORD
`ifdef VCS function void uvm_component::uvm_set_verbosity(string setting_comp,string setting_id,uvm_verbosity setting_verbosity,string setting_phase,time setting_offset);
  ...
  begin
    ...
    begin
      setting.comp = setting_comp;
      setting.id = setting_id;
      setting.verbosity = setting_verbosity;
      setting.phase = setting_phase;
...
`ifndef UVM_VERDI_NO_IMPLICIT_GET
    begin
       ...
       if ($cast(rit, r)) begin
         ...
         $sformat(apl_val," type=uvm_integral_t value=%0d",rit.read(this));
       end
       else begin
                ...
                if($cast(riu, r)) begin
                  ...
                  $sformat(apl_val," type=int unsigned value=%0d",riu.read(this));
                   ...
                   if ($cast(rap, r)) begin
                     ...
                     $sformat(apl_val," type=uvm_active_passive_enum value=%0d",rap.read(this));
                   end
                   else begin
...
function void uvm_component::m_set_cl_verb;
  ...
  // 9001487267
  if (verdi_verbosity_options!="")
      values.push_back(verdi_verbosity_options);
  //
...
//TYPE:Bug
//Change_id:Apply Verbosity fix
`ifdef UVM_APPLY_VERBOSITY_FIX
    int i;
