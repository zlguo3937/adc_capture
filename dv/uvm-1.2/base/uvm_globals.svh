...
//Return 1 when UVM quit count is reached.
function int get_error_max_count_reached();
  return set_error;
endfunction
...
// Modified by Verdi P10049895-129363
mailbox verdi_uvm_mailbox = new();
//
...
//normal function/task on ip is static
(* uvm_fgp_lock = "uvm_wait_for_nba_region" *)
task uvm_wait_for_nba_region;
...
`ifdef UVM_DO_NOT_USE_SSCANF  
      setting_offset = args[4].atoi();
`else
      void'($sscanf(args[4],"%d", setting_offset));
`endif  
      end
    end
...
// 9001487267
function void verdi_set_verbosity_before_simulation(logic [115199:0] options_bv);
  // _ALL_ can be used for ids
  // +uvm_set_verbosity=<comp>,<id>,<verbosity>,<phase|time>,<offset>
  // +uvm_set_verbosity=uvm_test_top.env0.agent1.*,_ALL_,UVM_FULL,time,800

    verdi_verbosity_options = uvm_bits_to_string(options_bv);
endfunction
//
`endif
`endif
