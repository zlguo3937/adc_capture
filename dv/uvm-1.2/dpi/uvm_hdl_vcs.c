...
static int uvm_hdl_max_width()
{
  ...
  if (vpi_chk_error(NULL)) {
      char * err_str = "get value: failed for uvm_pkg::UVM_HDL_MAX_WIDTH. You may not have sufficient PLI/ACC capabilites enabled  \n";
...
static int uvm_hdl_set_vlog_real(vpiHandle r, char* path, double value, PLI_INT32 flag)
{
  static int maxsize = -1;
  s_vpi_value value_s = { vpiRealVal, { 0 } };
...
#else
static int uvm_hdl_set_vlog_real(char *path, double value, PLI_INT32 flag)
{
  static int maxsize = -1;
 ...
 */
#ifdef VCSMX_FAST_UVM
static int uvm_hdl_get_vlog_real(vpiHandle r, char* path, double *value, PLI_INT32 flag)
{
  static int maxsize = -1;
  int size;
  s_vpi_value value_s = { vpiRealVal, { 0 } };
...
#else
static int uvm_hdl_get_vlog_real(char *path,double *value, PLI_INT32 flag)
{
  static int maxsize = -1;
  int  size;
  vpiHandle r;
...
#ifdef VCSMX_FAST_UVM
static int uvm_hdl_get_vhpi_real(vhpiHandleT r, char* path, /*vhpiEnumT *value*/ double* value)
{
    static int maxsize = -1;
    int size, rtn;
    vhpiValueT value_s;
    ...
    if (r == 0) {
        const char * err_str = "get: unable to locate hdl path (%s)\n Either the name is incorrect, or you may not have PLI/ACC visibility to that name";
        char buffer[strlen(err_str) + strlen(path) + 1];
        sprintf(buffer, err_str, path);
        m_uvm_report_dpi(M_UVM_ERROR, (char*)"UVM/DPI/HDL_GET_REAL", &buffer[0], M_UVM_NONE, (char*)__FILE__, __LINE__);
        return 0;  
    }
    else {
        if (maxsize == -1) {
        ...
        if (size > maxsize) {
            ...
            char buffer[strlen(err_str) + strlen(path) + (2*int_str_max(10))];
            sprintf(buffer, err_str, path, size, maxsize);
            m_uvm_report_dpi(M_UVM_ERROR, (char*)"UVM/DPI/VHDL_GET_REAL", &buffer[0], M_UVM_NONE, (char*)__FILE__, __LINE__);
            return 0;
        }
...
#ifdef VCSMX
#ifdef VCSMX_FAST_UVM
int uvm_hdl_get_mhdl_real(vhpiHandleT vhpiH, char *path, double *value) {
    vhpiValueT value1;
    static int maxsize = -1;
    int size;
    value1.format=vhpiRealVal;
...
#else
int uvm_hdl_get_mhdl_real(char *path, double *value) {
...
#ifdef UVM_VPI_CHECK
           if (vpi_chk_error(NULL)) {
               const char * err_str = "get value: unable to get value for (%s)\n Either the name is incorrect, or you may not have PLI/ACC visibility to that name";
               char buffer[strlen(err_str) + strlen(path)];
               sprintf(buffer, err_str, path);
               m_uvm_report_dpi(M_UVM_ERROR,
                       (char*) "UVM/DPI/HDL_GET",
...
int uvm_hdl_read_real(char *path, double *value)
{
#ifndef VCSMX
#ifdef VCSMX_FAST_UVM
    r = vpi_handle_by_name(path, 0);
    int res = uvm_hdl_get_vlog_real(r, path, value, vpiNoDelay);
    vpi_release_handle(r);
    return res;
#else
    return uvm_hdl_get_vlog_real(path, value, vpiNoDelay);
#endif
#else
#ifndef USE_DOT_AS_HIER_SEP
    mhpi_initialize('/');
    ...
    if (mhpi_get(mhpiPliP, h) == mhpiVpiPli) {
      ...
      return res;
        ...
        vhpiHandleT r = (vhpiHandleT) mhpi_get_vhpi_handle(h);
        //return uvm_hdl_get_mhdl(r, path,value);
        int res = uvm_hdl_get_vhpi_real(r, path, value);
        mhpi_release_handle(h);
        return res;
    }
    else {
    ...
    mhpi_release_parent_handle(h);
      return uvm_hdl_get_mhdl_real(path,value);
    }
    else {
#ifdef VCSMX_ERROR_ON_NULL_HANDLE
        const char * err_str = "set: unable to locate hdl path (%s)\n Either the name is incorrect, or you may not have PLI/ACC visibility to that name";
...
#ifdef VCSMX
#ifdef VCSMX_FAST_UVM
int uvm_hdl_set_mhdl_real(mhpiHandleT h, char *path, double value, mhpiPutValueFlagsT flags) 
{
...
#else
int uvm_hdl_set_mhdl_real(char *path, double value, mhpiPutValueFlagsT flags)
{
#ifndef USE_DOT_AS_HIER_SEP
    mhpi_initialize('/');
#else
 ...
 */
int uvm_hdl_deposit_real(char *path, double value)
{
#ifndef VCSMX
#ifdef VCSMX_FAST_UVM
    vpiHandle r = vpi_handle_by_name(path, 0);
    int res = uvm_hdl_set_vlog_real(r, path, value, vpiNoDelay);
    vpi_release_handle(r);
    return res;
#else
    return uvm_hdl_set_vlog_real(path, value, vpiNoDelay);
#endif
#else
#ifndef USE_DOT_AS_HIER_SEP
    mhpi_initialize('/');
    ...
    if (mhpi_get(mhpiPliP, h) == mhpiVpiPli) {
      ...
      int res = uvm_hdl_set_mhdl_real(h, path, value, mhpiNoDelay);
      return res;
    }
    else {
#ifdef VCSMX_ERROR_ON_NULL_HANDLE
    ...
    mhpi_release_parent_handle(h);
      ...
      mhpi_release_parent_handle(h);
      return uvm_hdl_set_mhdl_real(path, value, mhpiNoDelay);
     }
    else {
#ifdef VCSMX_ERROR_ON_NULL_HANDLE
      const char * err_str = "set: unable to locate hdl path (%s)\n Either the name is incorrect, or you may not have PLI/ACC visibility to that name";
 ...
 * or the VHPI, and set it to 'value'.
 */
int uvm_hdl_force_real(char *path, double value)
{
#ifndef VCSMX
#ifdef VCSMX_FAST_UVM
    vpiHandle r = vpi_handle_by_name(path, 0);
    int res = uvm_hdl_set_vlog_real(r, path, value, vpiForceFlag);
    vpi_release_handle(r); 
    return res;
#else
    return uvm_hdl_set_vlog_real(path, value, vpiForceFlag);
#endif
#else
#ifndef USE_DOT_AS_HIER_SEP
    mhpi_initialize('/');
    ...
    if (mhpi_get(mhpiPliP, h) == mhpiVpiPli) {
      ...
      int res = uvm_hdl_set_mhdl_real(h, path, value, mhpiForce);
      return res;
    }
    else {
#ifdef VCSMX_ERROR_ON_NULL_HANDLE
    ...
    mhpi_release_parent_handle(h);
      return uvm_hdl_set_mhdl_real(path, value, mhpiForce);
 ...
 */
int uvm_hdl_release_and_read_real(char *path, double value)
{
#ifndef VCSMX
#ifdef VCSMX_FAST_UVM
    vpiHandle r = vpi_handle_by_name(path, 0);
    int res = uvm_hdl_set_vlog_real(r, path, value, vpiReleaseFlag);
    vpi_release_handle(r);
    return res;
#else
    return uvm_hdl_set_vlog_real(path, value, vpiReleaseFlag);
#endif
#else
#ifndef USE_DOT_AS_HIER_SEP
    mhpi_initialize('/');
	...
	(mhpi_get(mhpiPliP, mhpiH) == mhpiVhpiPli)) {
      ...
      sprintf(buffer, err_str, path);
      m_uvm_report_dpi(M_UVM_ERROR,
                       (char*) "UVM/DPI/HDL_RELEASE_AND_READ_REAL",
                       &buffer[0],
                       M_UVM_NONE,
                       (char*)__FILE__,
 ...
 * or the VHPI, and release it.
 */
int uvm_hdl_release_real(char *path)
{
