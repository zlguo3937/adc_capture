...
`define UVM_IF_METHOD5_BEGIN(method_name,gp,delay,is_nonblocking,phase) \
       uvm_object __req__; \
       int __r_tr_h__; \
       int obj_size = 0; \
...
`define UVM_IF_METHOD5_END(method_name,gp,delay,is_nonblocking,phase, sync) \
       if (obj_size && $cast(__req__,gp)) \
...
`define UVM_TLM_NB_TRANSPORT_FW_IMP(imp, T, P, t, p, delay)              \
  function uvm_tlm_sync_e nb_transport_fw(T t, ref P p, input uvm_tlm_time delay); \
    uvm_tlm_sync_e sync; \
    `UVM_IF_METHOD5_BEGIN("nb_transport_fw",t,delay,1,p) \
    if (delay == null) begin \
    ...
    `UVM_IF_METHOD5_END("nb_transport_fw",t,delay,1,p,sync) \
    return sync; \
  endfunction
`else
`define UVM_TLM_NB_TRANSPORT_FW_IMP(imp, T, P, t, p, delay)              \
...
`define UVM_TLM_NB_TRANSPORT_BW_IMP(imp, T, P, t, p, delay) \
  function uvm_tlm_sync_e nb_transport_bw(T t, ref P p, input uvm_tlm_time delay);  \
    uvm_tlm_sync_e sync; \
    `UVM_IF_METHOD5_BEGIN("nb_transport_bw",t,delay,1,p) \
    if (delay == null) begin \
       `uvm_error("UVM/TLM/NULLDELAY", \
                  {get_full_name(), \
    ...
    `UVM_IF_METHOD5_END("nb_transport_bw",t,delay,1,p,sync) \
    return sync; \
  endfunction
`else
`define UVM_TLM_NB_TRANSPORT_BW_IMP(imp, T, P, t, p, delay) \
...
`define UVM_TLM_B_TRANSPORT_IMP(imp, T, t, delay)                        \
  task b_transport(T t, uvm_tlm_time delay);                              \
    `UVM_IF_METHOD5_BEGIN("b_transport",t,delay,0,0) \
    if (delay == null) begin \
       `uvm_error("UVM/TLM/NULLDELAY", \
                  {get_full_name(), \
    ...
    `UVM_IF_METHOD5_END("b_transport",t,delay,0,0,0) \
  endtask
`else
`define UVM_TLM_B_TRANSPORT_IMP(imp, T, t, delay)                        \
  task b_transport(T t, uvm_tlm_time delay);                              \
