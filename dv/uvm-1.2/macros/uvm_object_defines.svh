...
`define uvm_field_utils_begin(T) \
   ...
   begin \
    ...
    `ifdef UVM_EMPTY_MFLD_AUTO \
        $$empty_function_if_last(); \
    `endif
...
// --------------------
// Modified by Verdi 9001505562

`define m_uvm_record_qda_int(ARG, FLAG, SZ) \
  begin \
    if(!((FLAG)&UVM_NORECORD)) begin \
      int sz__ = SZ; \
      int max_array_num_limit = __m_uvm_status_container.recorder.verdi_get_array_limit(); \
      if (max_array_num_limit < 0) begin \
          if(sz__ == 0) begin \
          ...
          if(sz__ == 0) begin \
            ...
            if (max_array_num_limit == 0) begin \
                ...
                if (sz__ >= max_array_num_limit) begin \
                  for(int i=0; i<max_array_num_limit; ++i) begin \
                    __m_uvm_status_container.scope.set_arg_element(`"ARG`", i); \
                    if ($bits(ARG[i]) > 64) \
                     __m_uvm_status_container.recorder.record_field(__m_uvm_status_container.scope.get(), ARG[i], $bits(ARG[i]), uvm_radix_enum'((FLAG)&UVM_RADIX)); \
                    else \
