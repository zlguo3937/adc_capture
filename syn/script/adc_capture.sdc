...
#create_clock -name V_CLK_250M -period ${clk_period_250} -waveform [list 0 [expr 0.5 * ${clk_period_250}]] -add
...
create_generated_clock -name clk_rd_96 -master_clock adc_96_clk500_mux1 -source [get_pins u_analog_top/ADC_CLK500M] -add \
        [get_ports PAD21_CLK_RD] -divide_by 2 -combinational

# generate clk_rd_48
create_generated_clock -name adc_48_clk500_mux0 -master_clock adc_48_clk500 -source [get_pins u_analog_top/ADC48_CLK500M] -add \
        [get_pins u_digital_top/u_crg/u_clk_gen/u_clkmux_to_CLK500M/u_dont_touch_clkmux_mux/Z] -divide_by 1 -combinational

create_generated_clock -name clk_rd_48 -master_clock adc_48_clk500_mux0 -source [get_pins u_analog_top/ADC48_CLK500M] -add \
        [get_ports PAD21_CLK_RD] -divide_by 2 -combinational


# set_clock_groups
set_clock_groups -name all_clock_group -asynchronous          \
        -group { adc_96_clk500 adc_96_clk500_mux1 clk_rd_96 } \
        -group { adc_48_clk500 adc_48_clk500_mux0 clk_rd_48 }
...
set_clock_groups -physically_exclusive \
        -group { adc_96_clk500_mux1 }  \
        -group { adc_48_clk500_mux0 }
...
set_input_delay -max [expr ${clk_period_500} * 0.7] -clock adc_96_clk500_mux1 [get_ports PAD20_RSTN] -add
set_input_delay -min [expr ${clk_period_500} * 0.1] -clock adc_96_clk500_mux1 [get_ports PAD20_RSTN] -add

set_input_delay -max [expr ${clk_period_500} * 0.7] -clock adc_48_clk500_mux0 [get_ports PAD20_RSTN] -add
set_input_delay -min [expr ${clk_period_500} * 0.1] -clock adc_48_clk500_mux0 [get_ports PAD20_RSTN] -add

set_input_delay -max [expr ${clk_period_200} * 0.7] -clock clk_200m [get_ports PAD20_RSTN] -add
set_input_delay -min [expr ${clk_period_200} * 0.1] -clock clk_200m [get_ports PAD20_RSTN] -add
